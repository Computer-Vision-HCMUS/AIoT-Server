...
def _decode_pcm(source: str | Path) -> bytes:
    """Decode an audio source to mono signed 16-bit PCM at 16 kHz."""
    try:
            ...
            for frame in container.decode(stream):
                ...
                for pcm_frame in converted if isinstance(converted, list) else [converted]:
                    if pcm_frame is not None:
                        output.write(bytes(pcm_frame.planes[0]))
            return output.getvalue()
    except (av.error.FFmpegError, IndexError, OSError) as exc:
        raise HTTPException(status_code=502, detail="Media source could not be decoded") from exc


@router.get(
    "/stream/{media_id}",
    name="stream_media",
    summary="Stream an enabled media item with HTTP Range or raw PCM output",
    responses={
        200: {"content": {"audio/mpeg": {}, "application/octet-stream": {}}},
...
def stream_media(
    ...
    if parsed.scheme in {"http", "https"}:
        ...
        if format == "pcm":
            return Response(
                _decode_pcm(item.source_url),
                media_type="application/octet-stream",
                headers={"X-Audio-Format": "s16le", "X-Sample-Rate": "16000"},
            )
