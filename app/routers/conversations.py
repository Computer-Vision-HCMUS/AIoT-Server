...
def respond(
    ...
    # Safety filter
    safety_flag = _detect_safety_flag(payload.user_message or "")

    # Generate response
    response_text = _get_response(session.emotion_label, safety_flag, payload.user_message)
    next_action = _next_action(session.emotion_label, safety_flag)
    ...
    if session.user and session.user.consent_audio_storage and payload.user_message:
        user_message_summary = _summarize_user_message(payload.user_message, safety_flag)
