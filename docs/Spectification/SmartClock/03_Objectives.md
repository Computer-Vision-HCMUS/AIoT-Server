...
### Objective 1 Data Workflow

Objective 1 gồm hai use case cần mô tả rõ luồng trao đổi dữ liệu giữa **ESP32 Edge Device**, **FastAPI Server** và **PostgreSQL Database**:

* **Use-Case 1: Pomodoro Timer** — ESP32 lấy/cập nhật cấu hình Pomodoro từ server; server đọc/ghi dữ liệu trong bảng `timer_configs`; khi hoàn thành phiên, ESP32 gửi log phiên lên server; server ghi dữ liệu vào bảng `pomodoro_sessions`.
* **Use-Case 2: Luyện tập thuyết trình** — ESP32 ghi nhận thời gian và audio luyện tập; server lưu record vào bảng `seminar_recordings`; server xử lý kết quả đánh giá cơ bản và trả kết quả luyện tập về thiết bị.
...
### Workflow Diagram: Pomodoro Timer

```mermaid
flowchart TD
    User([Người dùng])

    subgraph ESP["ESP32 Edge Device"]
        E1[Chọn Pomodoro Mode]
        E2[Request cấu hình Pomodoro]
        E3[Hiển thị thời gian tập trung / nghỉ]
        E4[Tùy chỉnh cấu hình nếu cần]
        E5[Chạy bộ đếm Pomodoro cục bộ]
        E6[Ghi nhận phiên hoàn thành]
        E7[Hiển thị số phiên hoàn thành]
    end

    subgraph Server["FastAPI Server"]
        S1["GET /smartclock/timer-config"]
        S2["PUT /smartclock/timer-config"]
        S3["POST /smartclock/pomodoro-sessions"]
        S4[Validate request]
        S5[Trả response cho ESP32]
    end

    subgraph DB["PostgreSQL Database"]
        D1[(timer_configs)]
        D2[(pomodoro_sessions)]
    end

    User -->|Nhấn nút chọn chế độ| E1
    E1 --> E2
    E2 -->|GET request| S1
    S1 -->|SELECT config in DB| D1
    D1 -->|timer config| S1
    S1 -->|200 OK + config| E3

    E3 --> E4
    E4 -->|PUT request nếu người dùng đổi thời gian| S2
    S2 -->|UPSERT config in DB| D1
    D1 -->|updated config| S2
    S2 -->|200 OK| E4

    E4 --> E5
    E5 -->|Hết phiên tập trung / nghỉ| E6
    E6 -->|POST session log| S3
    S3 --> S4
    S4 -->|INSERT session in DB| D2
    D2 -->|session id| S4
    S4 --> S5
    S5 -->|201 Created + session data| E7
    E7 --> User
```

> **Ghi chú:** Nếu ESP32 mất kết nối Internet, bộ đếm Pomodoro vẫn chạy cục bộ. Dữ liệu phiên hoàn thành có thể được đồng bộ lên server khi thiết bị online trở lại.

---

## 3.2.2. Use-Case 2: Luyện tập thuyết trình
...
Tính năng Luyện tập thuyết trình cho phép người dùng trình bày thử trước thiết bị. Hệ thống hỗ trợ ghi nhận thời gian trình bày và có thể mở rộng thêm khả năng đánh giá tốc độ nói, độ rõ ràng hoặc mức độ tự tin khi thuyết trình.
...
| Main Flow       | Người dùng chọn chế độ luyện tập thuyết trình → bắt đầu trình bày → hệ thống ghi nhận dữ liệu → hiển thị kết quả sau khi hoàn thành |
...
flowchart LR
    ...
    subgraph SmartClock["SmartClock"]
        UC1([Chọn chế độ luyện tập thuyết trình])
...
### Workflow Diagram: Luyện tập thuyết trình
...
flowchart TD
    ...
    subgraph ESP["ESP32 Edge Device"]
        E1[Chọn chế độ luyện tập thuyết trình]
        E2[Thiết lập thời lượng mong muốn]
        E3[Bắt đầu ghi âm bằng microphone]
        E4[Đếm thời gian trình bày trên TFT]
        E5[Kết thúc bài luyện tập]
        E6[Upload audio và duration lên server]
        E7[Nhận kết quả luyện tập]
        E8[Hiển thị nhận xét / điểm cơ bản]
    end

    subgraph Server["FastAPI Server"]
        S1["POST /smartclock/seminar-recordings"]
        S2[Lưu file audio]
        S3[Tạo recording record]
        S4[Phân tích bài nói]
        S5[Cập nhật kết quả luyện tập]
        S6["GET /smartclock/seminar-recordings/{id}"]
    end

    subgraph DB["PostgreSQL Database"]
        D1[(seminar_recordings)]
    end
    ...
    E6 -->|POST request audio + duration| S1
    S1 --> S2
    S2 --> S3
    S3 -->|INSERT status=pending in DB| D1
    D1 -->|recording id| S3
    S3 -->|201 Created + id| E6
    E6 --> E7

    S3 --> S4
    S4 --> S5
    S5 -->|UPDATE score/result in DB| D1
    D1 -->|updated result| S5

    E7 -->|GET polling result| S6
    S6 -->|SELECT result in DB| D1
    D1 -->|status + scores + feedback| S6
    S6 -->|200 OK + result| E7
    E7 --> E8
    E8 --> User
```

> **Ghi chú:** Workflow này mô tả API nghiệp vụ cho use case luyện tập thuyết trình. Khi chưa có Internet, ESP32 vẫn có thể đo thời gian cục bộ nhưng không upload audio và không nhận được kết quả phân tích từ server.
...
| Objective 1: Hỗ trợ tập trung | Luyện tập thuyết trình | User | Luyện tập thuyết trình và quản lý thời lượng nói | Người dùng cải thiện kỹ năng trình bày và kiểm soát thời gian |
