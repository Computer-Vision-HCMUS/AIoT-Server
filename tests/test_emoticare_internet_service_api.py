...
def test_conversation_summary_is_not_persisted_without_consent(client, monkeypatch):
    _, headers, session_id = _create_flow(client, "PRIVATE-001", ["neutral"])
    monkeypatch.setattr(
        "app.routers.conversations.chat",
        lambda emotion, message, safety: "Cam on ban da chia se.",
    )

    response = client.post(
        "/api/conversations/respond",
        headers=headers,
        json={"session_id": session_id, "user_message": "Noi dung rieng tu cua toi"},
    )

    assert response.status_code == 200
    db = SessionLocal()
    try:
        assert db.query(ConversationRequest).one().user_message_summary is None
    finally:
        db.close()
...
def test_crisis_variants_are_detected(message):
    assert detect_safety_flag(message) == "high"


def test_crisis_response_directs_user_to_people_professionals_and_emergency_help():
    normalized = CRISIS_RESPONSE.lower()
    assert "người bạn tin cậy" in normalized
    assert "chuyên gia" in normalized
    assert "dịch vụ hỗ trợ khẩn cấp" in normalized
    assert "cấp cứu" in normalized


def test_unsafe_ai_output_is_rejected(monkeypatch):
    monkeypatch.setattr(
        "app.services.gemini.gemini_client.generate_text",
        lambda *args, **kwargs: "Bạn chắc chắn bị trầm cảm và hãy tăng 20 mg thuốc mỗi ngày.",
    )
    response = chat("sad", "Tôi thấy buồn hôm nay", "low")
    assert "chắc chắn bị" not in response.lower()
    assert "20 mg" not in response.lower()
...
def test_voice_conversation_accepts_raw_pcm(client, monkeypatch):
    _, headers, session_id = _create_flow(client, "VOICE-001", ["neutral"])

    async def fake_transcribe_pcm(pcm, sample_rate):
        assert pcm == b"\x00\x00" * 160
        assert sample_rate == 16000
        return SimpleNamespace(transcript="toi thay binh tinh", language="vi", duration_sec=0.01)

    monkeypatch.setattr(
        "app.routers.conversations.stt_service.transcribe_pcm_s16le",
        fake_transcribe_pcm,
    )
    monkeypatch.setattr(
        "app.routers.conversations.chat",
        lambda emotion, message, safety: "Hay hit tho cham mot chut nhe.",
    )

    response = client.post(
        f"/api/conversations/voice?session_id={session_id}&sample_rate=16000",
        headers={**headers, "Content-Type": "application/octet-stream"},
        content=b"\x00\x00" * 160,
    )

    assert response.status_code == 200
    assert response.json()["transcript"] == "toi thay binh tinh"
    assert response.json()["reply_text"]
    assert response.json()["audio_path"] == ""
